...
class InitializeData:
    ...
    async def generate_dept(self):
        ...
        生成部门详情数据
        """
        datas = self.datas.get("vadmin_auth_dept")
        for dept in datas:
            if dept.get("name") == "kinit 开发团队" or dept.get("name") == "质量管理部":
                dept["name"] = "质量管理部"
                dept["order"] = 0
                dept["parent_id"] = None
                dept["owner"] = None
                ...
                dept["email"] = None
                dept["disabled"] = False
            if dept.get("dept_key") == "total_group" or dept.get("dept_key") == "qc_dept":
                dept["dept_key"] = "qc_dept"
    ...
    async def generate_user(self):
        ...
        生成用户
        """
        datas = self.datas.get("vadmin_auth_user")
        has_test_user = False
        for user in datas:
            if user.get("telephone") == "15020221010" or user.get("telephone") == "10000000001":
                user["telephone"] = "18201855759"
                user["password"] = auth_models.VadminUser.get_password_hash("admin")
                user["name"] = "管理员"
                user["nickname"] = "管理员"
            elif user.get("telephone") == "13627135697":
                user["password"] = auth_models.VadminUser.get_password_hash("test_01")
                user["name"] = "测试用户"
                user["nickname"] = "测试用户"
                has_test_user = True
        ...
        if not has_test_user:
            import datetime
            test_user = datas[0].copy()
            test_user["id"] = 2
            test_user["telephone"] = "13627135697"
            test_user["password"] = auth_models.VadminUser.get_password_hash("test_01")
            test_user["name"] = "测试用户"
            test_user["nickname"] = "测试用户"
            test_user["create_datetime"] = datetime.datetime.now()
            test_user["update_datetime"] = datetime.datetime.now()
            datas.append(test_user)
            
            # 同时需要添加角色和部门关联
            self.datas.get("vadmin_auth_user_roles").append({"user_id": 2, "role_id": 1})
            self.datas.get("vadmin_auth_user_depts").append({"user_id": 2, "dept_id": 1})
